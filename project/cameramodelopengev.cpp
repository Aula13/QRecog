...
CameraModelOpenGEV::CameraModelOpenGEV()
{
    ...
    updateObserversTimer.setInterval(100);
    */
        ...
        if(app->discoverDevices()>0) {
            ...
            if(device.getStreamingChannelNumber()>0) {
                if(device.openStreamChannel(2)==GEV_STATUS_SUCCESS) {
                    ...
                    connect(observer, SIGNAL(pointCloudUpdate(const pcl::PointCloud<pcl::PointXYZRGBA>::Ptr)),
                            this, SLOT(update(const pcl::PointCloud<pcl::PointXYZRGBA>::Ptr)),
                            Qt::DirectConnection);
...
void CameraModelOpenGEV::stop()
{
    if(instanceFlag)
    {
        if(connected)
        {
            ...
            disconnect(observer, SIGNAL(pointCloudUpdate(const pcl::PointCloud<pcl::PointXYZRGBA>::Ptr)),
                       this, SLOT(update(const cloudType::Ptr)));
...
cloudPtrType CameraModelOpenGEV::getLastAcquisition()
{
...
void CameraModelOpenGEV::update(const pcl::PointCloud<pcl::PointXYZRGBA>::Ptr &cloud)
{
